...
public class ReaderView
	...
	private static final int FLING_MARGIN      = 50;  // 🚀 优化：减少滑动阻力，从100px降到50px
	private static final int GAP               = 20;

	private static final float MIN_SCALE        = 1.0f;
	private static final float MAX_SCALE        = 64.0f;

	private boolean mHorizontalScrolling = true; // 滑动方向，false=垂直，true=水平
	...
	public void onLongPress(MotionEvent e) {
		if (mTextSelectionEnabled && mTextSelectionManager != null) {
			PageView pageView = (PageView) getDisplayedView();
			if (pageView != null) {
				// 启动文本选择
				boolean success = mTextSelectionManager.startSelection(
					e.getX(), e.getY(), pageView
				);
				if (success) {
					// 禁用其他手势处理
					tapDisabled = true;
	...
	public boolean onScale(ScaleGestureDetector detector) {
		...
		// 通知缩放变化监听器
		if (mZoomChangeListener != null && Math.abs(mScale - previousScale) > 0.01f) {
			mZoomChangeListener.onZoomChanged(mScale);
		}
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		// 如果在文本选择模式，优先处理选择事件
		if (mTextSelectionManager != null && mTextSelectionManager.isInSelectionMode()) {
			PageView pageView = (PageView) getDisplayedView();
			if (pageView != null) {
				if (mTextSelectionManager.handleTouchEvent(event, pageView)) {
					return true;
				}
			}
		}
	...
	public void setAdapter(Adapter adapter) {
		...
		// 初始化文本选择管理器
		if (mTextSelectionManager == null && mAdapter != null) {
			MuPDFCore core = ((PageAdapter) mAdapter).getCore();
			if (core != null) {
				mTextSelectionManager = new TextSelectionManager(mContext, core, this);
			}
		}
	...
	private static int directionOfTravel(float vx, float vy) {
		// 🚀 优化：降低方向判断阈值，使滑动切换更容易
		// 原来是2倍，现在改为1.2倍，更容易触发页面切换
		if (Math.abs(vx) > 1.2f * Math.abs(vy))
			return (vx > 0) ? MOVING_RIGHT : MOVING_LEFT;
		else if (Math.abs(vy) > 1.2f * Math.abs(vx))
	...
	/**
	 * 设置滑动方向
	 * @param horizontal true=水平滑动(左右翻页), false=垂直滑动(上下翻页)
	 */
	public void setScrollDirection(boolean horizontal) {
		if (mHorizontalScrolling != horizontal) {
			mHorizontalScrolling = horizontal;
			// 重新布局以应用新的滑动方向
			requestLayout();
		}
	}
	...
	/**
	 * 获取当前滑动方向
	 * @return true=水平滑动, false=垂直滑动
	 */
	public boolean isHorizontalScrolling() {
		return mHorizontalScrolling;
	}
	...
	public void setZoom(float zoom) {
		float newZoom = Math.min(Math.max(zoom, MIN_SCALE), MAX_SCALE);
		if (Math.abs(newZoom - mScale) > 0.01f) {
			mScale = newZoom;
			requestLayout();
			// 通知监听器
	...
	public void fitToWidth() {
		if (getWidth() > 0 && getChildCount() > 0) {
			...
			if (child != null && child.getWidth() > 0) {
				float targetZoom = (float) getWidth() / child.getWidth();
				setZoom(targetZoom);
			}
		}
	}
	 ...
	 * 专用于文本选择等需要精确页面定位的场景
	 */
	public void fitToWidthAndCenter() {
		if (getWidth() > 0 && getChildCount() > 0) {
			// 获取当前显示的页面
			View currentView = getDisplayedView();
			if (currentView != null && currentView.getWidth() > 0) {
				// 计算适合宽度的缩放比例
				float targetZoom = (float) getWidth() / currentView.getWidth();
				setZoom(targetZoom);
					...
					public void run() {
						...
						if (cv != null) {
							...
							post(new Runnable() {
								@Override
								public void run() {
									View centerView = getDisplayedView();
									if (centerView != null) {
										slideViewOntoScreen(centerView);
									}
								}
							});
						}
	...
	public void cancelTextSelection() {
		if (mTextSelectionManager != null) {
			mTextSelectionManager.cancelSelection();
		}
	}
